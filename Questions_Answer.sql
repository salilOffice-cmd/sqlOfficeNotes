...
SELECT Projects.Title, Clients.ClientName
FROM Projects
LEFT JOIN Clients ON Projects.ClientId = Clients.ClientId;



SELECT Employee.Name, COALESCE(Projects.Title, 'Not assigned') AS ProjectTitle
FROM Employee
LEFT JOIN Projects ON Employee.Id = Projects.EmployeeId;
...
Sure, here are the solutions for the provided SQL practice questions:

Joins:

Retrieve the names of employees along with the titles of the projects they are assigned to.
...
FROM Employee
LEFT JOIN Projects ON Employee.Id = Projects.EmployeeId;
Perform a SQL join to fetch the projects and their corresponding client names.
...
FROM Projects
LEFT JOIN Clients ON Projects.ClientId = Clients.ClientId;
List all employees and their project titles, including those who are not assigned to any project.
...
FROM Employee
GROUP BY Department;
Find the total count of employees in each city.
...
FROM Employee
GROUP BY City;
Calculate the maximum age of employees in each department.
...
Retrieve the names of employees and their corresponding departments, replacing NULL department values with 'Not assigned'.

sql
Copy code
SELECT Name, ISNULL(Department, 'Not assigned') AS Department
FROM Employee;
Display the project titles and employee names. Replace NULL employee names with 'Unassigned'.

sql
Copy code
SELECT Projects.Title, ISNULL(Employee.Name, 'Unassigned') AS EmployeeName
FROM Projects
LEFT JOIN Employee ON Projects.EmployeeId = Employee.Id;
CASE Statement:

Show employee names and a column indicating whether their salary is above 50000 or not.
...
SELECT Name,
    CASE 
        ...
        ELSE 'Below or equal to 50000'
    END AS SalaryStatus
FROM Employee;
Use a CASE statement to categorize employees into 'Young' if their age is below 30, and 'Old' if 30 or above.
...
Retrieve the names of employees who are from London.
...
FROM Employee
WHERE City = 'London';
List the project titles for projects that have not started yet.
...
Calculate the average salary of employees for each department and display only those departments with an average salary greater than 40000.
...
Display the project titles and their total count. Show only projects with more than 2 employees assigned.
...
Show the project titles for the 5 most recently started projects.
...
List employees earning a salary greater than 45000 and aged between 25 and 30.
...
Display projects where the start date is after today.
...
FROM Projects
WHERE StartDate > GETDATE();
Find employees with a salary equal to or greater than 50000 and whose name contains 'James'.
...
Retrieve projects with titles starting with 'Develop' and having a client ID in (1, 2).
