...
CREATE TABLE CUSTOMER 
   (	CUST_CODE VARCHAR(6) NOT NULL PRIMARY KEY, 
	...
	GRADE int, 
	OPENING_AMT decimal(18,2) NOT NULL, 
	RECEIVE_AMT decimal(18,2) NOT NULL, 
	PAYMENT_AMT decimal(18,2) NOT NULL, 
	OUTSTANDING_AMT decimal(18,2) NOT NULL, 
...
CREATE TABLE  ORDERS 
   (
        ORD_NUM decimal(12,2) NOT NULL PRIMARY KEY, 
...
-- EXECUTE Below Procedure and Test it Read it 


/*
Title:			spDB_GetFilteredCustomerData
...
Comments:
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Example:		exec spDB_GetFilteredCustomerData    @PageSize = 20,
                                                     @PageNumber = 1,                                                          
											         @SortColumn = 'CUST_CITY',
											         @SortOrder  = 'desc'
...
ALTER PROCEDURE spDB_GetFilteredCustomerData        @AgentCode CHAR(6) = NULL,
                                                    @CustomerCity CHAR(35) = NULL,
                                                    @Grade INT = NULL,
                                                    @SortColumn NVARCHAR(50) = 'CUST_NAME',
                                                    @SortOrder NVARCHAR(4) = 'ASC', -- Default sort order is ascending
                                                    @PageSize INT = 10,
                                                    @PageNumber INT = 1
AS
BEGIN
...
/*Create a temporary table to hold paginated results*/
...
/*Common Table Expression (CTE) to filter and assign row numbers for pagination with sorting*/

;WITH SortedCustomers AS (
        SELECT ROW_NUMBER() OVER (ORDER BY CASE WHEN @SortOrder = 'ASC' 
		                                        THEN CASE @SortColumn WHEN 'CUST_CODE' THEN CUST_CODE
                                                                      WHEN 'CUST_NAME' THEN CUST_NAME
                                                                      WHEN 'CUST_CITY' THEN CUST_CITY
                                                                      WHEN 'GRADE' THEN CONVERT(NVARCHAR, GRADE)
                                                                      ELSE CUST_NAME -- Default to CUST_NAME if column name is invalid
                                                                      END
                                                                      END ASC,
        ...
        FROM   CUSTOMER
        WHERE  (@AgentCode IS NULL OR AGENT_CODE = @AgentCode) AND 
		       (@CustomerCity IS NULL OR CUST_CITY = @CustomerCity) AND 
			   (@Grade IS NULL OR GRADE = @Grade)
    )


INSERT INTO #TempCustomerData (RowNum, 
                               CUST_CODE, 
							   CUST_NAME, 
							   CUST_CITY, 
							   GRADE)
